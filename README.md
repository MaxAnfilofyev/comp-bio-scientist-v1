...
* Model provenance gate (publishable runs): `check_model_provenance` verifies every parameter in `{model_key}_spec.yaml` appears in `{model_key}_param_sources.csv` and is not labeled `free_hyperparameter`. `run_biological_model` enforces this by default; bypass with `--enforce_param_provenance` (or `AISC_ENFORCE_PARAM_PROVENANCE=false`). Results are logged to `project_knowledge.md` and summarized under Model Definitions in `provenance_summary.md`.
* Release snapshots (roadmap): planned `freeze_release(tag=...)` will capture repo state (commit + diff), key manifests (`project_knowledge.md`, `hypothesis_trace.json`, `claim_graph.json`, `provenance_summary.md`), referenced artifacts, and env info into `experiment_results/releases/{tag}/` (code archive, env manifest, release manifest with checksums/dirty flag). Typed manifest entries like `code_release_archive`, `env_manifest`, and `release_manifest` will make bundles discoverable for Zenodo/OSF uploads once implemented.
