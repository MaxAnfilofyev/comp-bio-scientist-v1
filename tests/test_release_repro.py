...
class DummyComputeModelMetricsTool:
    def use_tool(self, **kwargs):
        return {"output_csv": kwargs.get("input_path"), "model_metrics_json": None}


class DummyPlottingTool:
    def use_tool(self, **kwargs):
        return {"figure_path": str(Path(kwargs.get("output_dir")) / "plot.png")}


@pytest.fixture(autouse=True)
def patch_tools(monkeypatch):
    monkeypatch.setenv("AISC_ACTIVE_ROLE", "tester")
    monkeypatch.setattr(ao, "ComputeModelMetricsTool", DummyComputeModelMetricsTool)
    monkeypatch.setattr(ao, "RunBiologicalPlottingTool", DummyPlottingTool)
    yield


def _write_with_checksum(path: Path, content: str) -> str:
    path.write_text(content)
    return ao._safe_sha256(path)


def test_check_release_reproducibility_ok(tmp_path, monkeypatch):
    ...
    env_manifest.write_text(json.dumps({"python_version": "3.11"}))
    env_checksum = ao._safe_sha256(env_manifest)

    code_archive = release_root / "code_release.zip"
    code_checksum = _write_with_checksum(code_archive, "codezip")

    csv_path = release_root / "artifacts" / "sweep.csv"
    csv_path.parent.mkdir(parents=True, exist_ok=True)
    csv_checksum = _write_with_checksum(csv_path, "a,b\n1,2\n")

    solution_path = release_root / "artifacts" / "model_solution.json"
    solution_checksum = _write_with_checksum(solution_path, "{}")

    files = [
        {"path": ao._relative_to_release(env_manifest, release_root), "checksum": env_checksum},
        {"path": ao._relative_to_release(code_archive, release_root), "checksum": code_checksum},
        {"path": ao._relative_to_release(csv_path, release_root), "checksum": csv_checksum},
        {"path": ao._relative_to_release(solution_path, release_root), "checksum": solution_checksum},
    ]
    release_manifest = {
        "tag": tag,
