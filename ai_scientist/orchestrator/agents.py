...
def build_team(model: str, idea: Dict[str, Any], dirs: Dict[str, str]) -> Any:
    ...
    modeler = _make_agent(
        ...
        instructions=(
            ...
            "|-----------|-------------|\\n"
            "| Custom ODE equations | `create_ode_model` → auto-generates Python script + spec |\\n"
            "| Built-in model (predator_prey, etc.) | `save_model_spec` to register it |\\n"
            "| Pre-existing code with spec | `save_model_spec` to register it |\\n\\n"
            "### Phase 2: SIMULATION\\n"
            "- Build graphs: `build_graphs`\\n"
            "- Run models: `run_biological_model` (ODEs) or `run_comp_sim` (compartmental)\\n"
            "- Parameter exploration: `run_sensitivity_sweep`, `run_intervention_tests`\\n"
            "- Transport simulations: `run_transport_batch` (use `resolve_baseline_path` first)\\n\\n"
            "### Phase 3: POST-PROCESSING\\n"
            "- Run `sim_postprocess` on sim.json to produce arrays (.npy/.npz)\\n"
            "- Validate with `validate_per_compartment_outputs` before marking complete\\n"
            "- Compute metrics: `compute_model_metrics` or `save_simulation_metrics` for custom\\n\\n"
            "### Phase 4: LOGGING\\n"
            "- Update `update_hypothesis_trace` after each sim/ensemble\\n"
            "- Write `save_verification_note` for proof-of-work\\n"
            "- Update transport manifest: `update_transport_manifest`\\n"
            "- Log failures/reflections: `manage_project_knowledge`\\n\\n"
            "## EXAMPLES\\n\\n"
            "✓ **GOOD - Custom ODE model**:\\n"
            "```\\n"
            "create_ode_model(model_name='E2_model', variables=['M','E'], parameters=['A','C'], equations=['k_prod_M*E - k_decay_M*M', 'k_prod_E - k_consume_E*M*E'])\\n"
            "run_biological_model(model_key='E2_model', time_end=100, sweep_params={'A': [0.1, 0.5, 1.0]})\\n"
            "```\\n\\n"
            "✗ **BAD - Manual spec with equations**:\\n"
            "```\\n"
            "save_model_spec(model_key='E2_model', content_json='{\"equations\": ...}')  # Missing script_path!\\n"
            "```\\n\\n"
            "✓ **GOOD - Built-in model**:\\n"
            "```\\n"
            "save_model_spec(model_key='predator_prey', content_json='{...}', readme='Using built-in model')\\n"
            "run_biological_model(model_key='predator_prey', time_end=50)\\n"
            "```\\n\\n"
            "## ANTI-PATTERNS (AVOID)\\n\\n"
            "❌ **Calling save_model_spec for custom equations without script_path**\\n"
            "   → Use `create_ode_model` instead, it generates both script and spec\\n\\n"
            "❌ **Marking status=complete without validation**\\n"
            "   → Always run `validate_per_compartment_outputs` first\\n\\n"
            "❌ **Passing sim.json as baseline to transport**\\n"
            "   → Use `resolve_baseline_path`; only pass graph files (.npy/.graphml/.gpickle)\\n\\n"
            "❌ **Creating images (.png, .svg)**\\n"
            "   → Leave visualization to the Analyst agent\\n\\n"
            "❌ **Writing to user_inbox directly**\\n"
            "   → Log to Project Knowledge via `manage_project_knowledge`\\n\\n"
            "## ARTIFACT HELPERS\\n"
            "Use specialized helpers (NOT generic reserve):\\n"
            "- `create_transport_artifact`, `create_sensitivity_table_artifact`, `create_intervention_table_artifact`\\n"
            "- `save_verification_note` for proof-of-work\\n\\n"
    ...
    pi = _make_agent(
        ...
        instructions=(
            ...
            "| Simulation data / ODE models | `modeler` | model_spec.json, sim.json, metrics.json |\\n"
        ...
        tools=[
            ...
            *([getattr(__import__('ai_scientist.orchestrator.tool_wrappers', fromlist=['get_or_create_implementation_plan']), 'get_or_create_implementation_plan'),
            ...
            modeler.as_tool(
                tool_name="modeler",
                tool_description=(
                    "Execute computational modeling workflow. This agent will: create custom ODE models from equations "
                    "('create_ode_model'), save model specifications, run simulations (bifurcation/stochastic/transport), "
                    "execute parameter sweeps and intervention tests, compute metrics, update hypothesis_trace.json. "
                    "Use when you need simulation data or a new mathematical model defined. "
                    "Specify experiment ID (e.g., 'E1'), ODE equations if needed, input paths, and output directory."
                ),
                max_turns=role_max_turns,
