...
from ai_scientist.tools.biological_model import (
...
def run_biological_model(
    ...
    res = RunBiologicalModelTool().use_tool(
        ...
        model_spec_path=ledger.get("spec_path"),
    )
    _append_artifact_from_result(res, "output_json", metadata_json)
    try:
        if hypothesis_id and experiment_id:
...
def create_ode_model(
    ...
    """
    Generate a safe, compliant Python script for a custom ODE model from equations.
    
    Args:
        model_name: Name of the model (e.g. 'linear_feedback_model')
        variables: List of state variables (e.g. ['x', 'y'])
        parameters: List of parameters (e.g. ['k', 'a'])
        equations: List of string equations for d(var)/dt, matching variables order.
                   Math is standard python syntax (np.sin, etc allowed).
    """
    # 1. Generate Code
    try:
        code_content = create_ode_code(model_name, variables, parameters, equations)
    except Exception as e:
        raise ValueError(f"Failed to generate model code: {e}")

    # 2. Reserve Path for Script
    # Script artifact? We don't have a 'model_code' type yet, maybe use 'code'?
    # Or strict 'models/code'?
    # Use generic 'code' located in experiment_results/code/
    # We'll rely on generic file write or specific artifact reservation if type exists.
    # Check registry: 'code' type exists?
    # Registry has 'biological_model_solution', 'model_spec'.
    # For now, let's assume we can write to 'experiment_results/code/' manually or via new type.
    # To be safe and canonical, we should probably add 'model_code' to registry but user didn't ask for registry changes.
    # We will use 'write_text_artifact' style logic but restricted.
    
    # Actually, we can just define the path ourselves relative to experiment root?
    # But we want provenance.
    # Let's check if we can reserve 'model_spec' and put code next to it?
    # Or just use the 'code' folder as per convention in spec.
    
    # We will execute a write to 'experiment_results/code/{model_name}.py'.
    # And we'll use `RunBiologicalModelTool.use_tool` which expects a spec pointing to it.
    
    # So we need to:
    # A. Write the code file.
    # B. Write the spec file pointing to A.
    
    exp_dir = BaseTool.resolve_output_dir(None).resolve()
    if exp_dir.name == "experiment_results":
        code_dir = exp_dir / "code"
    else:
        code_dir = exp_dir / "experiment_results" / "code"
    code_dir.mkdir(parents=True, exist_ok=True)
    
    script_path = code_dir / f"{model_name}.py"
    try:
        with open(script_path, "w") as f:
            f.write(code_content)
    except Exception as e:
        raise RuntimeError(f"Failed to write model script at {script_path}: {e}")
        
    # 3. Create Model Spec
    spec_content = {
        "model_name": model_name,
        "description": f"Auto-generated model for {', '.join(variables)}",
        "variables": variables,
        "parameters": {p: 1.0 for p in parameters}, # Default values
        "equations": equations,
        "script_path": str(script_path.relative_to(Path.cwd())), 
        "notes": "Generated by create_ode_model."
    }
    
    # Use save_model_spec to register it properly
    # Note: save_model_spec takes string content
    return save_model_spec(
        model_key=model_name,
        content_json=json.dumps(spec_content),
        readme=f"Auto-generated ODE model {model_name}"
    )
