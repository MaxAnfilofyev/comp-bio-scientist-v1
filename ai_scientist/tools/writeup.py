...
from ai_scientist.tools.vlm_review import generate_vlm_img_review
...
def get_citation_addition(
    ...
    msg_history = []
    citation_system_msg_template = """You are an ambitious computational biology researcher who is looking to publish a paper to a top-tier ML conference that will contribute significantly to the field.
...
writeup_system_message_template = """You are an ambitious computational biology researcher who is looking to publish a paper that will contribute significantly to the field.
...
def perform_writeup(
    ...
    try:
        ...
        loaded_summaries = filter_experiment_summaries(exp_summaries, step_name="writeup")
        ...
        if not osp.exists(osp.join(latex_folder, "template.tex")):
            shutil.copytree(
                "ai_scientist/templates/blank_icml_latex", latex_folder, dirs_exist_ok=True
            )
        ...
        try:
            ...
            for pf in plot_names:
                ...
                try:
                    ...
                    if review_data:
                        # The return is a string (review), not a dict in vlm_review.py?
                        # Wait, let's check return type of generate_vlm_img_review in vlm_review.py
                        # It returns `review = response.choices[0].message.content` which is a string.
                        desc_map[pf] = review_data
                    else:
                        desc_map[pf] = "No description found"
                except Exception as e:
                    print(f"Failed to generate VLM review for {pf}: {e}")
                    desc_map[pf] = "No description found"
...
"""
        ...
        if banner_parts:
            try:
                ...
                meta.update(
                    {
                        "/ReleaseTag": release_tag or "N/A",
                        "/ReleaseCommit": release_commit or "N/A",
                        "/ReleaseDOI": release_doi or "N/A",
                        "/ReleaseEnv": release_env_checksum or "N/A",
                        "/ReleaseCode": release_code_checksum or "N/A",
                    }
                )
                ...
                print("Failed to inject PDF metadata (PyPDF2 error?)")
                print(traceback.format_exc())
